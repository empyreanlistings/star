...
async function initResponsiveVideo() {
  ...
  const updateVideoSource = () => {
    const isMobile = window.innerWidth <= 768;
    const videoSrc = isMobile ? 'images/mobile-video.mp4?v=3.56' : 'images/web-video.mp4?v=3.56';
...
function initPropertyModal() {
  ...
  if (!overlay.dataset.modalInit) {
    ...
    const open = () => {
      ...
      overlay.style.cssText = "display: flex !important; visibility: visible !important; opacity: 1 !important; z-index: 2147483.56 !important;";
...
async function loadComponent(selector, url, callback) {
  ...
  try {
    const response = await fetch(`${url}?v=3.56`);
